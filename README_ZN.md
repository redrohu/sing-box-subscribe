...
## VeCel 服务器处理时间的上限为10秒。如果 Clash 文件过大，将由于处理超时而失败
...
### 2023年11月30日更新：sing-box 1.8.0+ 已发布，需要修改配置。`rule_set` 模板已上传，默认生成最新的配置文件以适应 sing-box 1.8.0。如果您使用旧版本的 sing-box，请选择 `config_template_groups_tun.json` 模板。

使用 `/config/URL` 添加 `/&file=2` 参数以选择 `config_template_groups_tun.json` 模板。两个子链接的形式也可以跟随 `/&file=2` 参数。
